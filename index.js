...
const encdec = require("./encryptDecrypt");
//express app setup
...
app.post("/auth/user", async (req, res) => {
  ...
  const data = req.body;
  const user = await sqlConnection.query(
    "SELECT COUNT(1) FROM users WHERE `name` = ? AND `password` = ?",
...
app.post("/secured", async (req, res) => {
  const securedThingShit = req.body.message;
  const decrypted = encdec.decrypt(securedThingShit);
  console.log(decrypted, "recieved ");

  const ec = encdec.encrypt([
    {
  ...
  console.log(ec, "encryptedd before send");
  res.status(200).json({ message: ec });
});
